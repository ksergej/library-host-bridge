...
EXTRACT-USER.
    ...
    INSPECT WS-XML-REQUEST TALLYING WS-START FOR CHARACTERS BEFORE WS-TAG-USER-START.
    IF WS-START >= LENGTH OF WS-XML-REQUEST
        EXIT
    END-IF
    COMPUTE WS-START = WS-START + LENGTH OF WS-TAG-USER-START.
    INSPECT WS-XML-REQUEST TALLYING WS-END FOR CHARACTERS BEFORE WS-TAG-USER-END.
    IF WS-END <= WS-START
        EXIT
    END-IF
...
EXTRACT-BOOK.
    MOVE 0 TO WS-START WS-END WS-LEN.
    INSPECT WS-XML-REQUEST TALLYING WS-START FOR CHARACTERS BEFORE WS-TAG-BOOK-START.
    IF WS-START >= LENGTH OF WS-XML-REQUEST
        EXIT
    END-IF
    COMPUTE WS-START = WS-START + LENGTH OF WS-TAG-BOOK-START.
    INSPECT WS-XML-REQUEST TALLYING WS-END FOR CHARACTERS BEFORE WS-TAG-BOOK-END.
    IF WS-END <= WS-START
        EXIT
    END-IF
    COMPUTE WS-LEN = WS-END - WS-START.
