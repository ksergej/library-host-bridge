CBL NOXREF NOMAP NOOFFSET NOSOURCE
       ...
       01  WS-TAG-ACT-USR-ST    PIC X(8)  VALUE '<userId>'.
       ...
       MAIN-SECTION.
           ...
           PERFORM SHOW-MQ-RESULT
           MOVE 'REPLY SENT CORRELID=REQ MSGID' TO WS-TERM-MSG
           PERFORM SEND-TEXT.
       ...
       PARSE-REQUEST.
           MOVE SPACES
             TO WS-REQUEST-TYPE WS-USER-ID WS-BOOK-ID WS-LOAN-ID
       ...
       EXTRACT-ACTIVE-USER-ID.
           ...
           INSPECT REQ-DATA
               TALLYING WS-START
               FOR CHARACTERS BEFORE WS-TAG-ACT-USR-ST
           IF WS-START >= LENGTH OF REQ-DATA
              EXIT PARAGRAPH
           END-IF
           COMPUTE WS-START = WS-START + LENGTH OF WS-TAG-ACT-USR-ST
           INSPECT REQ-DATA
               TALLYING WS-END
               FOR CHARACTERS BEFORE WS-TAG-ACT-USR-END
           IF WS-END <= WS-START
              EXIT PARAGRAPH
           END-IF
           COMPUTE WS-LEN = WS-END - WS-START
       ...
       EXTRACT-LOAN-ID.
           ...
           INSPECT REQ-DATA
               TALLYING WS-START
               FOR CHARACTERS BEFORE WS-TAG-LOANID-ST
           IF WS-START >= LENGTH OF REQ-DATA
              EXIT PARAGRAPH
           END-IF
           COMPUTE WS-START = WS-START + LENGTH OF WS-TAG-LOANID-ST
           INSPECT REQ-DATA
               TALLYING WS-END
       ...
       BUILD-STUB-RESPONSE.
           ...
           IF WS-REQUEST-TYPE = 'ACTIVE'
              ...
              STRING
                 ...
                 FUNCTION TRIM(WS-STATUS-MESSAGE)
                 DELIMITED BY SIZE
                 '</message>' DELIMITED BY SIZE
